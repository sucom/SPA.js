...
function _fnValidIpv4(obj, msg){
  ...
  if (!isValid){
    isValid = ((/^(25[0-5]|2[0-4]\d|[01]?\d\d?)\.(25[0-5]|2[0-4]\d|[01]?\d\d?)\.(25[0-5]|2[0-4]\d|[01]?\d\d?)\.(25[0-5]|2[0-4]\d|[01]?\d\d?)$/i.test(elValue))
                && ( _.every(elValue.split('.'), function(part){ return ((part.length>1)? (!part.beginsWith("0")) : true); }) ));
  }
...
function _fnValidIpAddress(obj, msg) {
  ...
  { var strIpAddr = elValue;
    ...
    isValid = ((rxValidIp.test(strIpAddr)) && ( _.every(strIpAddr.split('.'), function(part){ return ((part.length>1)? (!part.beginsWith("0")) : true); }) ));
    _showValidateMsg(obj, msg, isValid);
...
function _fnValidSubnet(obj, msg) {
  ...
  if (!isValid) {
    ...
    isValid = ((rxSubnet.test(elValue)) && ( _.every(elValue.split('.'), function(part){ return ((part.length>1)? (!part.beginsWith("0")) : true); }) ));
  }
  return _showValidateMsg(obj, msg, isValid);
}
